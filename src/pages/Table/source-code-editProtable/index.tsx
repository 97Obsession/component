...
function TableRender<T extends Record<string, any>, U, ValueType>(
    ...
    // TableContext 统一管理了表格的核心状态和操作方法
    // 例如：表格主体可通过 counter.action 触发数据刷新等操作，列配置组件可通过 counter.columnsMap 获取列状态
    const counter = useContext(TableContext);

    /** 需要遍历一下，过滤掉不需要展示的数据，不然不支持嵌套表格 */
    ...
    // tableColumns为原始列配置，就平时我们传入的column数组
    // counter.columnsMap为列的映射状态，show：true ｜ false，这个是我们在列配置中设置的
    ...
    /** 自定义的 render */
    // 如果用户配置了tableViewRender属性，则可定义外部容器是什么，什么包裹住表格

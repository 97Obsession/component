...
>(props: EditableProTableProps<DataType, Params, ValueType>) {
    ...
    // 设置 ref
    // 第一个参数是父组件传入的ref
    // 第二个参数是返回需要暴露给父组件的内部ref
    // 第三个参数是依赖数组，当这个变化时，重新生成ref实例，返回给父组件
    useImperativeHandle(rest.actionRef, () => actionRef.current, [
        actionRef.current,
    ]);

    // useMergedState用于控制切换是否是受控组件，如果父组件不穿onchange和value，则内部接管，如果传，则由父组件接管
    ...
    const getRowKey = React.useMemo<
        GetRowKey<DataType>
    >((): GetRowKey<DataType> => {
        // 如果rowKey是一个函数，则直接返回调用
        if (typeof rowKey === 'function') {
            return rowKey;
        }
        // 如果是rowKey是字符串，则返回rowKey，不然就返回索引
    ...
    const coverRowKey = useRefFunction(
        (finlayRowKey: number | string): string | number => {
            /**
             * 如果不是 prop.name 的模式，就需要把行号转化成具体的rowKey。
             */
            // 非 name 模式下，将「索引」转换为「唯一键（rowKey）」
            if (typeof finlayRowKey === 'number' && !props.name) {
                // 如果输入的索引大于原始数据的大小，返回输入的索引，因为可能是新增的行，不能越界处理
                if (finlayRowKey >= value.length) return finlayRowKey;
                // 表格完整的数据
                const rowData = value && value[finlayRowKey];
                // 将行数据转换为其唯一jian
                // 例如：输入索引 2，通过 rowData = value[2] 拿到第 3 行数据，再通过 getRowKey(rowData, 2) 得到其 id（如 row-3），最终返回 row-3。
             ...
             */
            // 场景 2：name 模式下，将「唯一键（rowKey）」转换为「索引」为了更好的操作表单
            // 例如：输入唯一键 row-3，遍历 value 找到 getRowKey(item, index) === 'row-3' 的行，返回其索引 2。
            // 新增行
            if (
    ...
    return (
        <>
            {/*
            ProTable的actionRef是为了让父组件能够使用这个ref去调用表格的方法
            而通过context传递是因为，ProTable不会默认透传props到所有子节点
            如RecordCreator组件
            */}
            <EditableTableActionContext.Provider value={actionRef}>
                <ProTable<DataType, Params, ValueType>
                    ...
                    // 透传父组件传入的其他未显式声明的 props
                    {...rest}
                    // 传入动态生成的表格渲染配置
                    {...buttonRenderProps}
                    tableLayout="fixed"
                    actionRef={actionRef}
                    // 透传父组件的表格数据或状态变化时的回调（如排序、筛选、分页变化）
                    onChange={onTableChange}
                    editable={{
                        // 继承父组件传入的可编辑配置（如 onValuesChange 数据变化回调）。
                        ...editableProps,
                        // 绑定表单实例 formRef，用于控制表格行的表单状态（如获取 / 设置行数据），与上面的formref是同一个引用
                        formProps: {
                            // 传递引用容器本身，子组件可通过 formRef.current 访问实例
                            formRef,
                    ...
                    onDataSourceChange={(dataSource: readonly DataType[]) => {
                         ...
                         */
                        // 若表格绑定了表单名称（props.name），同步更新表单字段值，实现与 Form 组件的联动
                        if (props.name && position === 'top') {
                            // 如果在使用可编辑表格组件时，传入了name属性，且表格组件外层包裹了Form组件
                            const newValue = set(
                                {},
                                [props.name!].flat(1).filter(Boolean),
                                dataSource,
                            );
                            // 表单实例的 API，如 setFieldsValue、getFieldValue 等是 ProFormInstance 的方法
                            // formRef.current 是 引用容器中存储的实际表单实例（ProFormInstance），用于 调用实例的方法（如操作表单数据）
